...
public class DataExtractTask implements Runnable {
	...
	public void run() {
		...
		try {
			...
			while (getState().equals(STATE_RUNNING)) {
				...
				if (count <= end) {
					 list = fromPlat.fetch(from_connection, fromDatabase,
							select, count, end);
			...
			for (Object bean : list) {
				DynaBean b = null;
				b = (DynaBean) bean;
				String sqlstr = toPlat.getInsertSql(toDatabase, b);
	...
	public String getUserExtractSql() {
		String sql = RepositoryCache.getCurrentConfig().getUserExtractSql(
				fromDatabase.getName(), table.getName());
	...
	public void setUserExtractSql(String sql) {
		...
		userExtractSql = new UserExtractSql(fromDatabase.getName(),
				table.getName(), sql);
	...
	private void calcuteTotal() throws DataTransferException {
		...
		try {
			...
			String sql = "";
			sql = this.getUserExtractSqlCount();
