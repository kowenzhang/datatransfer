...
//        }
        ...
        if(isClosed||workQueue.size()==0){//注释以上代码，改为不等待任务
            return null;
        }
...
//            interrupt(); 	//中断线程池中的所有的工作线程,此方法继承自ThreadGroup类
    ...
    public void waitFinish() {
        ...
        int count = enumerate(threads); //enumerate()方法继承自ThreadGroup类，把此线程组及其子组中的所有活动线程复制到指定数组中
    ...
    private class WorkThread extends Thread {
        ...
        public void run() {
            while (!isInterrupted()) {  //isInterrupted()方法继承自Thread类，判断线程是否被中断
                System.out.println("workthread" + id + " 正在运行");
                Runnable task = null;
                try {
